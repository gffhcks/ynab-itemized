...
def validate_transaction_totals(transaction: ItemizedTransaction) -> Tuple[bool, List[str]]:
    ...
    calculated_discount = sum(item.discount_amount or Decimal("0") for item in transaction.items)
    ...
    ynab_amount = abs(transaction.ynab_transaction.amount / 1000)  # Convert from milliunits
    calculated_total = (
        calculated_subtotal + 
        calculated_tax - 
        calculated_discount + 
