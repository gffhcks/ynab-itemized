...
from ..models.transaction import YNABTransaction, TransactionStatus
from .exceptions import (
    YNABAPIError, YNABAuthError, YNABRateLimitError, 
    YNABNotFoundError, YNABValidationError
)
...
class YNABClient:
    ...
    def __init__(self, api_token: str = None, budget_id: str = None, base_url: str = None):
        ...
        try:
            ...
            if response.status_code == 400:
                ...
                raise YNABValidationError(
                    f"Validation error: {error_data.get('error', {}).get('detail', 'Unknown error')}",
                    status_code=response.status_code,
    ...
    def get_transactions(
        ...
        for t in transactions:
            try:
                ...
                logger.warning(f"Failed to parse transaction {t.get('id', 'unknown')}: {e}")
                continue
    ...
    def get_transaction(self, transaction_id: str) -> Optional[YNABTransaction]:
        ...
        try:
            response = self._make_request('GET', f'/budgets/{self.budget_id}/transactions/{transaction_id}')
